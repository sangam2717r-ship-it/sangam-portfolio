        ...
        <div className={`space-y-10 border-l-2 ml-3 ${isDark ? 'border-neutral-800' : 'border-neutral-200'}`}>
          {t.experienceSection.roles.map((role, idx) => (
            <div key={idx} className="relative pl-8 group">
              {/* Timeline Dot */}
              <span className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-indigo-500 ring-4 transition-transform group-hover:scale-125 ${isDark ? 'ring-[#0a0a0a]' : 'ring-neutral-50'}`}></span>
              
              {/* Content */}
              <span className={`text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full ${isDark ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-100 text-indigo-600'}`}>
                {role.period}
              </span>
